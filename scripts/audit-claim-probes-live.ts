...
import { PATENT_PARAM_ALIASES, expandParamAliases } from "../src/physics/paramAliases";
import * as fs from "node:fs";

const decSrc = fs.readFileSync(
  "src/components/patents/ClaimsDecoder.tsx",
  "utf8",
);
...
for (const [pid, map] of Object.entries(PATENT_PARAM_ALIASES)) {
  ...
  for (const spec of Object.values(map)) {
    if (!ctrlIds.has(spec.canonical)) {
      problems.push({ aliasEntry: pid, canonical: spec.canonical, issue: "canonical is not a control" });
    }
  }
}
console.log(
  JSON.stringify(
    { branches: branches.length, registryPatents: regIds.length, problems },
    null,
    2,
  ),
);
