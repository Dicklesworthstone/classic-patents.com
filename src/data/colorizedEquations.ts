...
import { applySupplementalColorizedEquations } from "./colorizedEquationSupplementalData";

const TEXTUAL_MATH_TOKEN_NAMES = new Set([
  "AR",
  "EV",
  "COP",
  "CTE",
  ...
  while (match !== null) {
    ...
    if (sym.startsWith("\\text{") || sym.startsWith("\\mathrm{") || sym.startsWith("\\mathbf{")) {
      const inner = sym
        ...
        .replace(/\}$/, "")
        .trim();
      if (TEXTUAL_MATH_TOKEN_NAMES.has(inner)) {
...
applySupplementalColorizedEquations(ALL_COLORIZED_EQUATIONS);
