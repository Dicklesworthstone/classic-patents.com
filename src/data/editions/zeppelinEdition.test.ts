...
describe("US 621,195 Ferdinand von Zeppelin Airship Archival Edition Contract", () => {
  test("is a valid, complete manual edition of US 621,195", () => {
    const result = validateCuratedSpecificationEdition(zeppelinArchivalEdition);
  ...
  test("pinned PDF SHA-256 matches archival edition", () => {
    expect(zeppelinArchivalEdition.sourcePdfSha256).toBe(PINNED_SHA256);
    const pdfPath = path.join(
      process.cwd(),
      "public",
      "patents",
      "pdfs",
      "us-621195-zeppelin-airship.pdf",
    );
  ...
  test("contains all 4 printed claims exactly matching manual claim text", () => {
    const claims = zeppelinArchivalEdition.blocks.filter((b) => b.kind === "claim");
  ...
  test("all figure preview assets exist on disk with exact pixel dimensions", () => {
    const figurePreviews = zeppelinArchivalEdition.blocks.flatMap((block) => {
  ...
  test("reviewed transcript ledger exists and contains page markers", () => {
    const transcriptPath = path.join(
      ...
      "patents",
      "transcripts",
      "us-621195-zeppelin-airship-reviewed.txt",
    );
    expect(fs.existsSync(transcriptPath)).toBe(true);
    const content = fs.readFileSync(transcriptPath, "utf-8");
    const matches = content.match(/--- REVIEWED TRANSCRIPTION PAGE \d+ OF 7 ---/g);
