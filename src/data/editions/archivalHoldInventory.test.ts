...
describe("archival hold inventory and partition contract (3hc.3)", () => {
  ...
  test("derives the exact catalogue totals and partition counts", () => {
    ...
    expect(report.acceptedCount).toBe(38);
    expect(report.heldCount).toBe(65);

    // Exact 65-state disjoint reason-code partition (after figure acceptance in 3hc.4):
    expect(report.categoryCounts["figure-related"]).toBe(45);
  ...
  test("proves every held patent delivers a complete source face in the reader", () => {
    ...
    expect(heldEntries.length).toBe(65);
  ...
  test("generates cleanly formatted markdown report containing all held entries", () => {
    ...
    expect(markdown).toContain("Fully Accepted Editions: 38");
    expect(markdown).toContain("Under Remediation Review: 65");
