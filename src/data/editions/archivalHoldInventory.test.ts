...
describe("archival hold inventory and partition contract (3hc.3)", () => {
  ...
  test("derives the exact catalogue totals and partition counts", () => {
    ...
    expect(report.acceptedCount).toBe(52);
    expect(report.heldCount).toBe(51);

    // Exact 51-state disjoint reason-code partition (after figure acceptance):
    expect(report.categoryCounts["figure-related"]).toBe(31);
  ...
  test("proves every held patent delivers a complete source face in the reader", () => {
    ...
    expect(heldEntries.length).toBe(51);
  ...
  test("generates cleanly formatted markdown report containing all held entries", () => {
    ...
    expect(markdown).toContain("Fully Accepted Editions: 52");
    expect(markdown).toContain("Under Remediation Review: 51");
