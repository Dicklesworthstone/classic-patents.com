...
describe("US 3,353,115 Theodore H. Maiman Ruby Laser Archival Edition publication contract", () => {
  ...
  test("keeps every printed figure occurrence semantic while withholding unaccepted crops", () => {
    ...
    const authoredRefs = maimanRubyLaserArchivalEdition.blocks.flatMap((block) =>
      block.kind === "paragraph"
        ? block.inlines.filter((inline) => inline.kind === "reference").map((inline) => inline.text)
        : [],
    );
    expect(
      [
        ...new Set(
          authoredRefs.flatMap((value) => [...value.matchAll(/\d+/g)].map((m) => Number(m[0]))),
        ),
      ].sort((a, b) => a - b),
