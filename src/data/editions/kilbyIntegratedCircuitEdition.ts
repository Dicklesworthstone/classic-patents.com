...
const sourceReference = (label: string, src?: string): CuratedSpecificationInline => ({
  ...
  ...(src
    ? { figurePreviews: [{ src, alt: `Primary-facsimile crop of ${label}`, width: 760, height: 560 }] }
    : {}),
});
...
export const kilbyIntegratedCircuitArchivalEdition: WithheldKilbyEdition = {
  ...
  completeFacsimileReviewed: false,
  blocks: [
    { kind: "masthead", lines: ["June 23, 1964", "United States Patent Office", "3,138,743", "MINIATURIZED ELECTRONIC CIRCUITS", "Jack S. Kilby, Dallas, Tex., assignor to Texas Instruments Incorporated, Dallas, Tex., a corporation of Delaware", "Filed Feb. 6, 1959, Ser. No. 791,602", "Patented June 23, 1964", "25 Claims. (Cl. 317-101)"] },
    { kind: "figure-sheet", figureLabel: "Figures 1, 1a, 2, 2a, 3, 4, 5, and 5a", title: "4 Sheets—Sheet 1", description: [sourceText("The first drawing sheet visibly labels Fig. 1, Fig. 1a, Fig. 2, Fig. 2a, Fig. 3, Fig. 4, Fig. 5, and Fig. 5a. Its reference numerals include 10, 10a, 10b, 11, 11a, 12, 12a, 13, 15, 15a, 16, 17, 17a, 18, 19, 20, 21, 22, 23, 25, 26, 27, 28, 29, 30, 35, 36, 37, and 38, with N, P, and N OR P labels.")] },
    { kind: "figure-sheet", figureLabel: "Figures 6a and 6b", title: "4 Sheets—Sheet 2", description: [sourceText("The second drawing sheet visibly labels Fig. 6a and Fig. 6b, with T1, T2, R1 through R8, C1 through C4, INPUT-1, INPUT-2, OUTPUT-1, OUTPUT-2, +V, -V, GND., and contact and lead numerals 50, 51, 52, 53, 54, 56, 60, and 70.")] },
    { kind: "figure-sheet", figureLabel: "Figure 7", title: "4 Sheets—Sheet 3", description: [sourceText("The third drawing sheet visibly labels Fig. 7, the conventional multivibrator wiring diagram, including T1, T2, R1 through R8, C1, C2, 400-ohm and 1.8K resistor values, 50 microfarads, INPUT-1, INPUT-2, OUTPUT-1, OUTPUT-2, +V, -V, and GND.")] },
    { kind: "figure-sheet", figureLabel: "Figures 8a, 8b, and 8c", title: "4 Sheets—Sheet 4", description: [sourceText("The fourth drawing sheet visibly labels Fig. 8a, Fig. 8b, and Fig. 8c, the phase-shift oscillator embodiment and its two corresponding wiring presentations. No later Figures 9 through 20 are printed in this grant.")] },
    { kind: "heading", level: 2, text: "SPECIFICATION" },
    sourceParagraph(sourceText("This invention relates to miniature electronic circuits, and more particularly to unique integrated electronic circuits fabricated from semiconductor material.")),
    ...
    sourceParagraph(sourceText("It is another principal object of this invention to produce desired circuits by appropriately shaping a wafer of semiconductor material to obtain the necessary isolation between components thereof and to define the areas utilized by such components.")),
    ...
    sourceParagraph(sourceText("It is a still further object of this invention to provide novel miniaturized electronic circuits fabricated as described above which involve less processing than techniques heretofore used for this purpose.")),
    ...
    sourceParagraph(sourceText("The foregoing and other objects and features of the invention will become more readily apparent from the following detailed description of preferred embodiments of the present invention when taken in conjunction with the appended drawings, in which: "), sourceReference("FIGURES 1-5a"), sourceText(" illustrate schematically various circuit components; "), sourceReference("FIGURE 6a"), sourceText(" illustrates schematically a multivibrator circuit; "), sourceReference("FIGURE 6b"), sourceText(" shows its wiring diagram in the same relationship; "), sourceReference("FIGURE 7"), sourceText(" illustrates the conventional wiring diagram; "), sourceReference("FIGURE 8a"), sourceText(" illustrates a phase shift oscillator; "), sourceReference("FIGURE 8b"), sourceText(" shows its wiring diagram in the same relationship; and "), sourceReference("FIGURE 8c"), sourceText(" portrays the wiring diagram of the phase shift oscillator.")),
    ...
    sourceParagraph(sourceText("Referring now to the drawings in detail, preferred embodiments of the present invention will now be described in detail in order that a better understanding of the principles of the invention and the various forms and embodiments of the invention will be better understood.")),
    ...
    sourceParagraph(sourceText("Resistor and capacitor designs may be combined to form a distributed R-C network. Such is shown in "), sourceReference("FIGURE 3"), sourceText(", wherein a wafer 20 of p-type conductivity having an n-type conductivity diffused layer 21 formed therein is provided with a broad area contact 22 on the face and spaced contacts 23 on the opposite face. These networks are useful for low pass-filters, phase shift networks, coupling elements, etc. Their parameters may be calculated from the equations above. Other configurations of this general type are also possible.")),
    sourceParagraph(sourceText("Transistors and diodes may be formed on a wafer, as described by Lee in Bell System Technical Journal, vol. 35, p. 23 (1956). This reference describes a transistor, as shown in "), sourceReference("FIGURE 4"), sourceText(", which has a collector region 25, a diffused p-n junction 26, a base layer 27, an emitter contact 28 for a rectifying connection with base layer 27 and base and collector contacts 29 and 30, respectively. The base layer 27 is formed as a mesa of small cross section.")),
    sourceParagraph(sourceText("A diode of similar design is shown in "), sourceReference("FIGURE 5"), sourceText(", and consists of a region 35 of one type conductivity, a mesa region 36 of opposite conductivity type with a p-n diffused junction formed therebetween and contacts 37 and 38 to each region. Small inductances, suitable for high frequency use, may also be made by shaping the semiconductor as evidenced by "), sourceReference("FIGURE 5a"), sourceText(" which shows a spiral of semiconductor material. It is also possible to prepare photosensitive, photoresistive, solar cells and other like components utilizing the considerations outlined above.")),
    ...
    sourceParagraph(sourceText("Because all of the circuit designs described above can be formed from a single material, a semiconductor, it is possible by physical and electrical shaping to integrate all of them into a single crystal semiconductor wafer containing a diffused p-n junction, or junctions, and to process the wafer to provide the proper circuit and the correct component values. Junction areas for the transistors, diodes, and capacitors are formed by properly shaped mesas on the wafer.")),
    sourceParagraph(sourceText("A specific illustration of an electronic circuit embodying the principles of the invention is shown in "), sourceReference("FIGURE 6a"), sourceText(". As shown, a thin wafer of single crystal semiconductor material containing a diffused p-n junction has been processed and shaped to include a complete and integrated multivibrator electronic circuit formed essentially in one surface of the wafer. The regions of the wafer have been marked with symbols representative of the circuit element functions that are performed in the various regions. "), sourceReference("FIGURE 6b"), sourceText(" shows a wiring diagram of the various circuit functions in the relationship which they occupy in the wafer of FIGURE 6a. A more conventionally drawn circuit diagram is shown in "), sourceReference("FIGURE 7"), sourceText(" with the circuit values actually used. The multivibrator circuit shown in FIGURES 6a, 6b and 7 will be described as illustrative of the processing techniques employed.")),
    ...
    sourceParagraph(sourceText("After this step, the photoresist was removed with a solvent and the mesa areas 60 masked by the same photographic process. The wafer was again immersed in etchant and the n layer completely removed in the exposed areas. A chemical etch is considered preferable. The photoresist was then removed.")),
    ...
    sourceParagraph(sourceText("A further illustration of the process hereof is shown in "), sourceReference("FIGURES 8a-8c"), sourceText(". Each area of the single crystal semiconductor wafer has been marked with a symbol for the circuit element which it represents. This unit illustrates the use of resistors, transistors, and a distributed R-C network to form a complete phase shift oscillator.")),
    ...
    sourceParagraph(sourceText("INVENTOR: Jack S. Kilby. By Stevens, Davis, Miller & Mosher, Attorneys.")),
  ],
};
...
export function manualKilbyClaimText(claimNumber: number): string {
  ...
  if (block?.kind !== "claim") throw new Error(`Kilby manual edition is missing claim ${claimNumber}.`);
  return block.inlines.map((inline) => inline.text).join("");
}

export const kilbyIntegratedCircuitParallelReadings: Readonly<Record<number, readonly string[]>> = {
  6: ["The opening identifies the subject as miniature electronic circuits fabricated from semiconductor material."],
  7: ["The patent explains that shrinking and packing discrete components was approaching its practical limit."],
  8: ["Conventional resistor miniaturization required a long sequence of substrate, material, termination, heat, and protection operations."],
  9: ["Capacitor, transistor, and diode processes were similarly numerous and often mutually incompatible."],
  10: ["Separate assembly could reduce process interactions, but assembly itself could damage the sensitive components."],
  11: ["Many operations, materials, tests, terminations, and alignment constraints lowered yield and reliability."],
  12: ["The invention's departure is to miniaturize with as few materials and compatible operations as possible."],
  13: ["A semiconductor body with diffused opposite-conductivity regions forms the junction structure that hosts every circuit element."],
  14: ["The components lie at or near one face of a thin wafer, and shaping supplies geometry and isolation."],
  15: ["Removal, geometry, diffusion, and conductivity conversion confine current paths and make the final circuit planar."],
  16: ["The patent states its principal object of integrating all components into a semiconductor body containing diffused junctions."],
  17: ["A second object is to shape the wafer so components are isolated and their occupied areas are defined."],
  18: ["A further object is a smaller, simpler package than prior circuit assemblies."],
  19: ["Another object is to produce the integrated circuits with fewer processing operations than earlier methods."],
  20: ["The primary object combines active and passive components near one body face while separating them as needed."],
  21: ["The drawing list distinguishes component figures, the multivibrator drawings, and the phase-shift oscillator drawings."],
  22: ["Active elements act as current generators in an impedance network; passive elements include resistors, capacitors, and inductors."],
  23: ["The source defines a circuit as electrically connected discrete elements, while acknowledging incidental transistor resistance and capacitance."],
  24: ["The detailed embodiments use an appropriately shaped single-crystal semiconductor body containing p-n junctions."],
  25: ["Germanium, silicon, gallium arsenide, aluminum antimonide, indium antimonide, and other materials are suitable."],
  26: ["The component figures establish the single-crystal semiconductor context and list germanium, silicon, gallium arsenide, aluminum antimonide, and indium antimonide as examples."],
  27: ["Figure 1 uses bulk resistance between ohmic contacts, governed by active length, cross-section, and resistivity."],
  28: ["Figure 1a confines current in a diffused n-type region and permits resistance adjustment by etching and doping."],
  29: ["Figure 2 uses transition capacitance at a diffused p-n junction and identifies the variables in its relation."],
  30: ["Figure 2a uses a semiconductor plate, an inert dielectric such as silicon oxide, and an evaporated metal plate."],
  31: ["Junction capacitors are diodes and are voltage-dependent; back-to-back regions provide a less polarized arrangement."],
  32: ["Figure 3 combines resistive and capacitive regions into useful distributed low-pass, phase-shift, and coupling networks."],
  33: ["Figure 4 identifies the collector, junction, base, emitter, base contact, and collector contact of a mesa transistor."],
  34: ["Figure 5 identifies a mesa diode, while Figure 5a shows a shaped spiral that can serve as a small high-frequency inductance."],
  35: ["Double diffusion can create both n-p-n and p-n-p structures, with suitable materials and established processes."],
  36: ["Physical and electrical shaping integrates the component types in one crystal and uses mesas to form junction areas."],
  37: ["The wafer embodiment in Figure 6a is a complete multivibrator, and Figure 6b preserves the same physical wiring relationship."],
  38: ["The fabrication example begins with polished 3 ohm-cm p-type germanium, antimony diffusion, and a precisely cut thin wafer."],
  39: ["Kovar leads, evaporated gold contacts, and aluminum emitter areas establish the electrical interfaces."],
  40: ["Photoresist and etching form isolation slots and the calculated resistor shapes, with electrolytic etching preferred."],
  41: ["A second mask and etch exposes mesa areas and removes the n layer where isolation requires it."],
  42: ["Thermally bonded gold wires complete the connections, while masked oxide and deposited gold provide an alternative interconnect."],
  43: ["Testing may be followed by hermetic sealing; the limited process sequence produces a compact, reliable, inexpensive device."],
  44: ["Figures 8a through 8c apply resistors, transistors, and distributed R-C material to a complete phase-shift oscillator."],
  45: ["The two embodiments are examples among innumerable circuits, bounded by the component types and values that fit in the space."],
  46: ["The specification reports more than thirty million components per cubic foot compared with five hundred thousand before the invention."],
  47: ["The closing technical paragraph preserves the inventor's allowance for equivalent changes that do not depart from the inventive concepts."],
  74: ["The cited references list the earlier United States patents named in the file of this patent."],
  75: ["The closing signature matter identifies Jack S. Kilby as inventor and Stevens, Davis, Miller and Mosher as attorneys."],
};
