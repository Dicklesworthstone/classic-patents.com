...
const sourceFigures = `/patents/figures/${PATENT_ID}`;
...
function sourceFigure(number: number, sourceText = `Fig. ${number}`): CuratedSpecificationInline {
  const cropByFigure: Record<number, string> = {
  ...
  const source = cropByFigure[number];
  if (!source) throw new Error(`US 2,846,084 has no authored crop for Fig. ${number}.`);
  return figure(
    sourceText,
    `#fig-${number}`,
    `Pinned source crop for Fig. ${number}`,
    `${sourceFigures}/${source}`,
  );
}

function sectionReference(sourceText: string, id: string, label: string): CuratedSpecificationInline {
  return {
    kind: "reference",
    text: sourceText,
    href: `#${id}`,
...
const blocks: CuratedSpecificationBlock[] = [
  ...
  { kind: "heading", level: 3, text: "In the drawings" },
  {
    ...
    description: [
      sourceFigure(1),
      text(" is an elevational view of a master assembly of the remote-control manipulator of the present invention; "),
      sourceFigure(2),
  ...
  },
  {
    ...
    description: [
      ...
      sourceFigure(3),
      text(" and partly in elevation, and shows the mounting of cable guides at the interconnection of horizontal and vertical arms; "),
      sourceFigure(5),
  ...
  },
  {
    ...
    description: [
      ...
      sourceFigure(8),
      text(" is an elevational view illustrating the mounting of certain assemblies on the support."),
    ],
  },
  {
    kind: "figure-sheet",
    ...
    description: [
      sourceFigure(9),
      text(" is a diagrammatic perspective view illustrating the cables that respond to opening and closing of the handle, other movements thereof with respect to the lower arm on which it is mounted, and rotation of the lower arm about its own axis."),
    ],
  },
  {
    kind: "figure-sheet",
    ...
    description: [
      sourceFigure(10),
      text(" is an elevational view, partly in section, of two gear boxes connected with certain cables shown in "),
      sourceFigure(9),
      text("; "),
      sourceFigure(11),
  ...
  },
  {
    ...
    description: [
      sourceFigure(13),
      text(" is a horizontal sectional view taken through the lower gear box on the line 13–13 of "),
      sourceFigure(12),
      text("; "),
      sourceFigure(14),
  ...
  },
  {
    ...
    description: [
      sourceFigure(15),
      text(" is a diagrammatic view showing one of a plurality of duplicate electrical systems interconnecting the master and slave units of the manipulator."),
    ],
  },
  {
    kind: "figure-sheet",
  ...
  ),
  {
    ...
    inlines: [
      {
        kind: "text",
        text: "In Fig. 15 are shown in schematic form the elements of each of the assemblies 54, 55, 56, 57, 58, 59, or 60 on the master unit and its corresponding assembly on the slave unit and the electrical connections between the units. It will be understood that there are seven systems like the one indicated in Fig. 15, a system such as shown in Fig. 15 reproducing each of the various components of motion included in any operation performed by the operator. As previously pointed out, each of the aforesaid assemblies consists of a gear box 78, 90, 99, or 100, a motor 204, a tachometer 205, and a transducer 209 or 209a. Each transducer on the master unit is a ",
      },
      {
        kind: "term",
