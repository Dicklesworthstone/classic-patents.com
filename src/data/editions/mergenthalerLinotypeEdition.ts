...
/** Preserve the printed citation as authored source semantics. Later figure
 * crops remain intentionally unbound until the drawing-sheet review accepts
 * them; accepted crops are attached automatically when present on disk. */
const sourceParagraph = (value: string): CuratedSpecificationBlock => {
  const inlines: CuratedSpecificationInlines = [];
  const pattern = /\bFigs?\. \d+[a-z]?(?:, \d+[a-z]?)*(?:,? (?:and|to) \d+[a-z]?)?/g;
  let cursor = 0;
  for (const match of value.matchAll(pattern)) {
    const start = match.index ?? 0;
    if (start > cursor) inlines.push(text(value.slice(cursor, start)));
    inlines.push(sourceFigure(match[0]));
    cursor = start + match[0].length;
  }
  if (cursor < value.length) inlines.push(text(value.slice(cursor)));
...
const mergenthalerLinotypeClaimSeed: PatentClaim[] = [
  {
    number: 1,
    isIndependent: true,
    originalText:
...
Alarm mechanism.—A bell, l, and a pivoted elastic striker, n, are mounted on the main frame. The striker is actuated by a spring, n, and is provided at the rear end with two arms, o′ and p′, having studs which act on the feeding and locking dogs f′ and f″ of the indicator, to release them and permit the return of the indicator at the instant that the bell is sounded a second time to show the completion of the line. The striker is lifted away from the bell and set for action by a vertical slide, q′, having on its edge, as in Figs. 29 and 31, an inclined surface to act against a rigid arm on the striker. As the striker is lifted, its arm is engaged and held by a pivoted spring-actuated elbow-lever, r′, having a shoulder, s′, on its side.

The indicator-rod g has at one end two projections, t′ and u′, of different lengths. When the line is at a certain distance from completion, the stud t′ encounters the detent-lever r′ and disengages its end from the striker, which falls until it encounters the shoulder s′, when it springs or yields sufficiently to strike the bell, but immediately rises out of contact therewith. As the line reaches completion, the stud u′ operates the detent-lever and disengages the shoulder, whereupon the striker is permitted to fall against the bell and sound the same a second time, this action being accompanied by the release of the indicator-dogs and the return of the indicator, as already explained.

The vertical slide q′, before referred to as the means of setting the striker, has at its extreme upper end an incline, i, on its side, so that when fully depressed this incline will trip the detent and cause the striker to fall in order that the indicator may be returned to the starting-point. This is for the purpose of enabling the operator, in the event of an error, to restore all the parts to their initial positions, so that the line may be commenced anew. The slide q′ is sustained normally in its elevated position, as shown in Fig. 15, by an arm, w′, on a rock-shaft, a′, in bearings in the main frame. This shaft, which extends in a fore-and-aft direction, also carries an arm, f, which acts against the rack-bar V to return the adjusting-pin frame K to the starting-point at the left.

The shaft a′ is further provided with an arm, z, united by a swivel-joint to an arm, a′, on a transverse rock-shaft, b′, to which the reversing-lever M is also attached, as in Figs. 1, 3, 16, and 17. In consequence of this connection and the other connections already described, the lever M is caused to serve the several purposes of moving the stop-pin frame and retracting plate, of restoring the indicator, and of restoring the adjusting-pin frame. In short, it restores all the parts in an instant to the condition for the commencement of a new line. The lever is maintained in its elevated position by means of a weight applied to its rear end, as shown in Figs. 16 and 17. This weight also overbalances the weight of frame o, which shifts the adjusting-frame, so that it is sustained in its elevated position.`),
    sourceParagraph(`Casting mechanism.—The casting mechanism embraces as the leading features, first, devices to align and clamp the matrix-bars; second, a sectional mold which is closed in an operative position in front and against the matrix-bars to present the molten metal to the line of selected characters to give form to the resulting bar or cast; and, third, appliances for melting the type-metal and delivering the same forcibly into the mold and against the matrix-bar.

Referring first to the mechanism for aligning and clamping the matrix-bars, attention is directed particularly to Figs. 1, 11, 32, 33, 34, 45, and 49. A horizontal clamping-bar, A′, extends transversely of the machine in position to extend across and against the rear edges of the entire series of matrix-bars when they are located in an operative position. This clamping-bar has suitably-formed arms which are connected to the main frame by means of dovetailed guides, as represented at d′, Figs. 1 and 45, whereby the bar is permitted to move forward and backward to and from the matrix-bars.

At the opposite side of the position occupied by the matrix-bars I arrange the horizontal sectional mold X X′, consisting of two horizontal bars, one overlying the other, at a distance therefrom equal to the thickness of the required printing-bar. Each of these mold-sections may be worked from the solid in a single piece, or, for convenience of construction, composed, as shown in Fig. 35, of parts or pieces inseparably united by rivets, screws, or other fastenings. Each of the mold sections or slides is formed, as shown in Fig. 35, with a shoulder, f, which serves as the ends of the mold.

Referring to Fig. 35, it will be seen that when brought into the relation shown, the two parts of the mold inclose a flat rectangular space, which is of a size and form corresponding with that of the printing-bars to be produced. By moving the mold-sections endwise with respect to each other the length of the mold proper may be increased or diminished, and thus the machine adapted to produce printing-bars of different lengths. By separating the mold-sections in an endwise direction, as represented in Fig. 34, the mold is opened in order to release and permit the removal of the casting formed therein.

As shown in Figs. 34 to 38, and also in Fig. 45, the upper mold-section is dovetailed to and arranged to slide on the under side of a guide-bar, h, bolted rigidly to the main frame, while the lower mold-section, X′, is arranged to slide in like manner on a guide, i, also bolted to the main frame. The location of the two guides is such that when the mold is closed to an operative position, it will stand directly across the front or operative edges of the series of matrix-bars, presenting its open side in front of the line of selected characters.

The opposite or rear side of the mold is closed by the flat mouth or face of a melting-pot, Y, which has a narrow slit or opening, j, for the delivery of molten metal into the mold directly toward the matrix-bars at the opposite side thereof. The melting-pot is fixed in the top of a box or chamber, C, which also contains a burner, D′, by means of which the pot is heated and the metal maintained constantly in a molten condition.

The box C is connected by horizontal pivots k at its lower side to the main frame, so that it may tip forward and backward to and from the mold. At its upper side the box C is connected by links l to upper ends of levers m′ on a transverse rock-shaft, n′, mounted in the arms of the sliding clamping-bar A′. It follows as a result of this arrangement that when the levers m′ are moved in the direction indicated by the arrow in Fig. 45, the clamping-bar A′ will be moved forward, so as to force the matrix-bars tightly against one side of the mold, after which the mouth of the melting-pot will be drawn firmly against the mold on the opposite side. Their reverse motion permits the melting-pot and matrix-bars to separate from the mold in order that the latter may be opened and the matrix-bars readjusted.

In order to prevent the entrance of molten metal between adjacent faces of the matrix-bars, they must be clamped firmly together previous to casting. For this purpose I employ laterally-acting clamps o′ and p′, which slide on horizontal guides located at opposite sides of the machine on the face of the clamping-bar A′, as plainly represented in Figs. 11, 32, 33, and 34. These clamps are connected by horizontal pins q to the respective mold-sections, so that when moved inward to act against the sides of the bars they carry the mold-sections to an operative position.

The inner ends or faces of the clamps stand in line with the shoulders forming the inner ends of the mold, as indicated by the dotted lines in Fig. 50, so that when inward movement of the clamps is arrested by contact with the matrix-bars, the ends of the mold are also brought accurately in line with the faces of the outer matrix-bars, and the length of the mold must coincide precisely with the length of the line of selected or assembled characters. The employment of the matrix-bars which are in use as a means of determining the length of the extensible mold is an important feature of the invention.

The clamps o′ and p′ are operated, as shown in Figs. 11, 32, and 34, by links connecting them with the upper ends of levers r and s, pivoted to the main frame and actuated by cam-wheels t, the wheels being grooved to engage studs or rollers at the extremities of crank-shafts u, secured on the shafts v′ which carry the levers last mentioned. The clamp p′ is moved positively in both directions, and under any given adjustment advances always to the same position. The clamp o′, at the opposite side, is actuated with a yielding pressure in order that it may accommodate itself to the varying width of the series of bars which may be brought into action between the two clamps. This yielding action is secured by mounting its operating-arm u′ loosely on its shaft v′ and connecting the two by means of a spiral spring, as shown in Fig. 49, or by any equivalent yielding connection.`),
    sourceParagraph(`Previous to clamping the matrix-bars it is necessary that they shall be aligned with great accuracy. For the purpose of effecting a preliminary alignment, I provide the lateral clamp o′ with a horizontal pin, x, projecting inward in advance thereof, so that as the clamp advances toward the bars the pin will first be thrust through the holes in the entire series of bars and until it enters a hole in the clamp at the opposite side of the machine. This aligning-pin is ordinarily pointed at the ends and made of such size as to fit loosely within the perforations of the bars. If demanded, however, it may fit closely within the bars and thus serve as the sole means of alignment. As the clamp recedes to release the bars, the aligning-pin is withdrawn from the bars.

In order to avoid danger of the bars swinging or moving laterally in such manner as to bind upon or engage with the aligning-pin, I make the face y of the clamp o′ separate from the remaining portion, so that it may slide independently thereon, as represented in Figs. 32, 34, and 50. This independent section, commonly designated the stripper-plate, is provided with a friction-spring a, bearing against the guide, as represented in Figs. 33 and 34, or is combined with other suitable friction devices which will render it necessary to apply a considerable force in order to effect its movement.

As a result of this construction, the stripper-plate will remain in position and prevent lateral movement of the matrix-bars during the backward movement of the clamp and aligning-pin, thus insuring the retraction of the pin without difficulty. The backward movement of the stripper-plate a sufficient distance to permit the descent of all the matrix-bars in advance thereof is secured by means of a spindle z′, extending therefrom loosely through the clamp o′, with a head at the outer end, as shown in Fig. 32. This spindle permits the necessary independent or lost motion between the clamp and stripper-plate, but causes the former to retract the latter at the completion of its movement.

As a means of perfecting the alignment of the bars vertically, previous to closing the clamping devices thereon, I mount in the clamping-bar A′ a horizontal plate, a′, arranged to slide forward and backward therein, having one edge adapted to enter the notches or indentations in the rear edges of the matrix-bars. When forced forward into these notches, the aligning-plate a′ insures the accurate adjustment of the designated characters, so that in the resulting cast they will form a straight line. The plate a′ is advanced for action by a cam, b′, applied to the shaft of the clamp-operating levers n′, the cam being so adjusted as to advance the aligning-plate before the clamping-bar is closed tightly against the matrix-bar.

Mention was previously made of the fact that the mold opens horizontally to permit delivery of the completed bar. The upper section, X, of the mold is provided on its under face, as shown in Figs. 36, 37, 38, and 44, with a longitudinal rib, c, which enters the top of the casting to retain the same against lateral displacement. This portion of the mold is further provided, as shown in the same figures, with a depending stud, d′, which enters the upper side of the casting to insure its passage endwise with the upper portion of the mold as the latter is opened.

When the mold is opened, the casting is carried to the right, adhering beneath the upper part of the mold. When the casting has been carried entirely clear of the lower part of the mold, its delivery is effected by means of a plate, e′, pivoted, as shown in Figs. 1 and 11, to swing in a vertical direction, its position being such that as it descends its edge will engage the rear edge of the bar and drive the same downward away from the top of the mold. Each casting thus delivered falls upon a shelf or support, f′, as shown in Fig. 34, in advance of a pin, g, extending to the lower part of the mold at the opposite side of the machine. As the mold recloses, the pin g will push the casting formed at the previous operation endwise from the shelf f′, each casting being delivered in rear of the one previously formed, so that they push one another from the machine.

The delivery-plate e′ is connected in rear of its pivot to a rod, i, which serves as a weight to maintain the forward edge of the plate normally in an elevated position, and which has its upper end provided with a head and extended through an arm or guide on the lifting-head p, by which the matrix-bars are lifted. As this head completes its upward movement, so that the matrix-bars assume their normal positions, it lifts the rod i′ and causes the same to operate the plate e′.

In order that the printing-bars may have flat faces and a uniform height, it is deemed advisable, although not absolutely necessary, to plane or dress their lower edges or bases. For this purpose I mount on the main frame a depending knife or cutter, j′, as shown in Figs. 2, 11, and 34, in such position that the base of the casting will be carried past its edge in the act of delivery. In this connection the rib c′ on the upper part of the mold is an important feature, in that it retains the casting securely and causes it to be carried in a right line past the cutter.

For the purpose of preventing accumulation of metal at the delivery mouth or outlet of the melting-pot, I provide the lower portion of the mold with an upright blade or wiper, k′, as shown in Figs. 34, 39, 40, and 41, usually a thin plate of spring-steel, which is carried over the mouth of the pot as the mold is opened and closed.

To insure speedy action and formation of a perfect cast, I propose to employ, in connection with the melting-pot, a pump or forcing apparatus to deliver the metal therefrom into the mold. I recommend, as the most simple and reliable known to me at the present time, a pump of the character represented in Fig. 45. The delivery duct or channel f of the melting-pot, having at its mouth a long exceedingly narrow slit or opening, is extended downward to the bottom of the pot and terminates in an upright cylindrical chamber provided near the top with an inlet-opening, m, through which the metal enters. In the top of this chamber there is mounted a vertically-reciprocating piston, n′. When this piston is raised above the opening m, the metal passes thereunder. When the piston is depressed, it closes the inlet-opening and delivers the metal through the opening j to the mold.

The depression of the piston is effected by means of a spiral spring, o′, applied to its upper end, as shown in Figs. 1 and 34. This use of the spring is advantageous in that it insures the application of a constant pressure, and avoids danger of breakage which would otherwise arise if metal were solidified beneath the piston at commencement of operations. The elevation of the piston is effected by an arm, p′, mounted on a rock-shaft having a depending arm, g′, the lower end of which is acted upon by a lifting-cam, r′, as plainly shown in Fig. 45.

For increasing the capacity of the machine I propose to provide the frame adjacent to the molds with passages through which water or other cooling mediums may be passed to reduce the temperature of the parts.`),
    sourceParagraph(`Details of drawing-gear.—The machine receives motion, primarily, through a band-pulley, c′, which revolves loosely on a tubular shaft, d′, carrying a fixed crank-arm, e′. The inner face of the pulley carries a stud or projection, f′, while the crank carries an axially-sliding pin, g′, designed to engage the stud for the purpose of imparting motion from the pulley through the crank to the tubular shaft. The pin g′ is notched transversely, so that when it stands in the position shown it will permit the stud to pass and the pulley to revolve without driving the shaft.

The clutch-pin g′ is attached to an arm, which is in turn connected to a rod or spindle, h, sliding longitudinally within the tubular driving-shaft. The inner end of the spindle is connected by a pin passing through the slits in the shaft to an external collar, f′, which is grooved circumferentially and connected by a pin or its equivalent to a controlling or shipping lever, k′, the motion of this lever in one direction serving to engage the clutch-pin and its movement in the opposite direction effecting disengagement.

The shipping-lever is operated in the first instance to engage the clutch by means of a hand-lever, l, which extends to the front of the machine in position to be operated by the attendant. The parts are held in connection for the proper length of time by means of a wheel, i, acting against a roller at the lower end of the shipping-lever, this wheel being provided in one side with a notch into which the roller may fall, as shown in Fig. 46, to permit the disconnection of the clutch. A worm-wheel, m, applied to the shaft of the wheel i, is turned by a worm on the end of the driving-shaft.

When the hand-lever l is operated to engage the clutch, motion is communicated from the shaft to the wheel i, which is at once turned so as to carry its notch away from the roller of the shipping-lever, the effect of which is to retain the clutch in engagement until the wheel i has made a complete revolution and again brought the notch beneath the roller, whereupon the parts automatically disengage. The other parts of the machine are so timed that this single rotation completes the production of a printing-bar. The shaft A′, which carries the wheel i, also carries a cam, r″, for operating the pump.

To effect the tilting of the melting-pot, as before described, the wheel i is provided in one side, as in Fig. 45, with a cam-groove, n, which engages a roller on a lever, o, the lower end of which is connected by a rod, p, to the lever m′, which actuates the main clamping-bar, the tilting-pot, and the rear aligning-bar.

To effect the automatic adjustment of the stop-pin frame and retracting-plate, which are operated from lever M, I connect said lever q to one end of a lever, r″, which is in turn pivoted to the main frame and raised and lowered at suitable intervals by means of a cam, s″, mounted on the shaft carrying the cams before alluded to. The lifting-head P is carried, as shown in Fig. 3, by an arm, t, sliding in a dovetailed groove in the upright portion of the main frame. It is sustained and elevated to lift the bars by means of a chain, u, attached to and arranged to wind upon a pulley, v, mounted on a horizontal shaft in the top of the main frame.

This shaft is provided, as shown in Fig. 1, with a pin, w, operated by a vertical rack-bar, x, the lower end of which is attached to an operating-lever, y, lifted at proper intervals by a cam, z, mounted on the shaft A′.

It will be observed that all of the driving cams and wheels are mounted upon the single transverse shaft.`),
    sourceParagraph(`Operation.—The parts stand normally in the position represented in Fig. 1, the head P being in its elevated position, and the entire series of matrix-bars suspended thereby at their highest elevation. At this time weight w acts to urge the adjusting-pin frame toward the right, the indicator mechanism stands at zero, the alarm mechanism is set for action, and the stop-pins stand in their normal positions—that is to say, in line with each other—as represented in Figs. 22 and 24.

The operator now depresses in succession the various keys representing the characters to be employed in the order in which the characters are to appear. At the termination of each word, or at other suitable points, he depresses the spacing-bar. At the depression of the first key the corresponding adjusting-pin forces backward the appropriate stop-pin to arrest the motion of the first bar. When the key is released, the adjusting-pin frame moves toward the right, bringing the adjusting-pins opposite the second matrix-bar, so that at the operation of the second key the stop-pin to arrest the second matrix-bar is projected to the rear. This operation is repeated until a suitable number of spaces and characters has been designated.

The aggregate value in width of the characters and spaces thus designated appears on the indicating mechanism at the front. At a suitable time, near completion of the line, the alarm mechanism gives notice that the line is nearly completed, and the operator, inspecting the indicator, determines whether there is sufficient room remaining to be filled by additional letters or characters. If not, he observes on the scale the width of the space to be filled, and, knowing the number of bars indicated for spacing purposes, presses the foot-lever c² in order to permit movement of the adjusting-pin frame to the left, and repeatedly depresses the particular spacing-key representing a spacing-surface of suitable width.

The result is to cause projection of the stop-pins representing spaces on the bars already designated for spacing. At completion of this operation there are sufficient stop-pins projected to arrest all bars called into use, but the pins do not yet project sufficiently rearward to effect stoppage. The operator now depresses lever Z, setting the automatic mechanism in action. The first effect is to move the stop-pin frame rearward to bring the rear ends of the stop-pins into position to arrest the matrix-bars, as indicated in Fig. 20. The lifting-head P now commences its descent, lowering the entire series of matrix-bars, which are individually arrested by their respective stop-pins.

As the movement of each bar is arrested, the lifting-head descending therefrom permits the dog Q of that bar to engage the rear guide-plate, i′, and sustain the bar after disengagement of the stop-pin. After the matrix-bars have been adjusted, the clamps o′ and p′ advance from opposite sides to clamp the bars between them. The clamp p′ reaches its limit of movement in advance of the other, so that on one side the series of bars stand always at the same point. The clamp o′, continuing its movement, applies a yielding pressure to the series of bars, which are thus confined snugly together irrespective of their number or width.

As clamp o′ advances, the aligning-pin thereon is advanced through the bars, whereby they are brought to substantially the required adjustment previous to action of the clamps. At or about the time that the lateral clamps act on the bars, the aligning bar or blade a′ enters the grooves in their rear edges, thus perfecting alignment. This action is immediately followed by advance of clamping-bar A′ from one direction and of the melting-pot from the opposite direction. At this time the matrix-bars are locked firmly in position on all sides, with the mouth or delivery-port of the melting-pot in intimate contact with the mold.

Immediately after the foregoing operation the piston n′ descends, delivering molten metal forcibly into the mold, where it immediately assumes the form of a printing-bar, the edge of which bears in relief the letters or characters in the sequence and arrangement in which they are to be printed. The melting-pot and clamping-bar A′ and aligning-bar a′ now retreat, and at or about the same time the lateral clamps are moved apart, withdrawing the aligning rod or pin from the bars.

The retraction of the lateral clamps is accompanied by opening of the mold, the upper portion of which carries the newly-formed bar to one side of the machine, where it is detached from the mold by plate e′, as before explained. During the casting operations the retracting-plate I is moved in such manner as to draw all the stop-pins backward within their supporting-frame, so that they no longer engage the heads and slides of the matrix-bars, which are thereafter sustained wholly by dogs Q. After retraction of the pins, the pin-frame is moved bodily forward, carrying the entire series of stop-pins to their normal position, so that operation of the finger-keys to designate characters for the second line may be carried on without interfering with casting of the first line.`),
    sourceParagraph(`Modifications.—While I prefer to employ continuous or rigid matrix-bars, as hereinbefore described, it is to be distinctly understood that I may substitute therefor bars composed of short sections jointed together after the manner of a chain, these jointed bars being adapted for operation in connection with the other parts substantially in the same manner as the rigid bars hereinbefore described.

It is also to be understood that in place of the matrix-bars I may substitute, in connection with the casting mechanism, matrix-wheels arranged to operate in essentially the same manner as the wheels described in Fig. 40 of my application for Letters Patent filed on the 10th day of July, 1884, No. 137,225.

While practical experience has shown that molten type-metal is the best material known in the art at the present day for formation of the printing-bars, it is to be understood that I may substitute therefor any plastic material adapted to be forced into the mold for formation of printing-bars, whether the material hardens or solidifies within the mold or requires additional treatment, such as baking, after delivery therefrom.

The matrix-bars herein described constitute an important feature of my invention. While it is preferred to use them in connection with mechanism such as herein described for adjusting and clamping them and for taking an impression from the aligned characters, it should be understood that they are adapted for use in machines other than the one shown, and indeed that for special purposes they can be adjusted and operated by hand, the advantages arising from the tapering form, the presence of spacing-surfaces between the characters, and the other features of novelty being in no wise dependent on the other parts with which they may be used. For these reasons, and because the invention in this regard resides in the individual bar rather than in duplication of the first bar, I claim herein the form of the individual bar.

I am aware that an attempt has been made to connect separate dies or matrices, each representing a single character, by means of thin flexible bands or cords to which they were attached. It is to be noted that my bar is continuous; that it possesses considerable stiffness or rigidity, so that it may be pushed or pulled endwise without difficulty; that the characters are formed directly in the bar and in close proximity to each other, instead of being widely separated, as was necessary with dies united by flexible connections.

I believe myself to be the first to produce a solid or continuous matrix-bar having the characters impressed or indented directly therein, and the first to produce a matrix-bar presenting continuous or practically continuous side faces, so that when arranged side by side the bars may be moved past each other without collision and without the necessity of intermediate guides. I also believe myself to be the first to provide a matrix-bar with spacing-surfaces elevated above the level of the characters therein, and the first to locate spacing-surfaces at suitable points between the characters, so that a bar may be stopped at different points in the course of its descent to present a blank surface at the aligning-point.

I also believe myself to be the first to combine with a changeable or convertible matrix—that is to say, a matrix composed of a series of dies or individual matrices adapted for transposition or rearrangement—a mold and a casting mechanism, and it will be manifest to the skilled mechanic that the construction of the mold and of the mechanism for delivering the metal therein may be modified without changing substantially the mode of connection or departing from the limits of my invention.`),
    { kind: "heading", level: 2, text: "Claims" },
    paragraph(literal("Having thus described my invention, what I claim is:")),
    ...mergenthalerLinotypeClaimSeed.map((claim) => ({
      kind: "claim" as const,
      number: claim.number,
...
const manualClaimText = (number: number): string => {
  ...
  if (block?.kind !== "claim") {
    throw new Error(`Mergenthaler manual edition is missing claim ${number}.`);
  }
  return block.inlines.map((inline) => inline.text).join("");
};

/** Claim decoders are metadata; every legal literal is read from the edition. */
export const mergenthalerLinotypeClaims: PatentClaim[] = mergenthalerLinotypeClaimSeed.map(
  ({ number, isIndependent, dependsOn, plainEnglish, keyInnovations, legalSignificance }) => ({
    number,
    isIndependent,
    ...(dependsOn ? { dependsOn } : {}),
