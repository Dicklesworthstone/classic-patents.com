...
const FESSENDEN_WIRELESS_ID = "us-706737-fessenden-wireless";
const FESSENDEN_WIRELESS_ASSETS = Object.keys(
  ARCHIVAL_FIGURE_ACCEPTANCE_ATTESTATIONS[FESSENDEN_WIRELESS_ID].assets,
);
const FESSENDEN_WIRELESS_OCCURRENCES = Object.fromEntries(
  FIGURE_OCCURRENCE_SOURCE_LOCATORS[FESSENDEN_WIRELESS_ID].map((locator) => [
    locator.occurrenceKey,
    locator.activeAsset,
  ]),
);
...
const DIESEL_ENGINE_ID = "us-542846-diesel-engine";
const DIESEL_ENGINE_ASSETS = Object.keys(
  ARCHIVAL_FIGURE_ACCEPTANCE_ATTESTATIONS[DIESEL_ENGINE_ID].assets,
);
const DIESEL_ENGINE_OCCURRENCES = Object.fromEntries(
  FIGURE_OCCURRENCE_SOURCE_LOCATORS[DIESEL_ENGINE_ID].map((locator) => [
    locator.occurrenceKey,
    locator.activeAsset,
  ]),
);
...
describe("figure occurrence source locators", () => {
  ...
  test("binds each Fessenden Wireless citation to its independently reviewed full source sheet", () => {
    const locators = FIGURE_OCCURRENCE_SOURCE_LOCATORS[FESSENDEN_WIRELESS_ID];
    expect(locators).toHaveLength(13);
    expect(locators.map((locator) => locator.occurrenceKey)).toEqual(
      Object.keys(FESSENDEN_WIRELESS_OCCURRENCES) as FigureOccurrenceKey[],
    );
  ...
  test("binds every Diesel citation to its independently reviewed full source sheet", () => {
    const locators = FIGURE_OCCURRENCE_SOURCE_LOCATORS[DIESEL_ENGINE_ID];
    expect(locators).toHaveLength(45);
    expect(locators.map((locator) => locator.occurrenceKey)).toEqual(
      Object.keys(DIESEL_ENGINE_OCCURRENCES) as FigureOccurrenceKey[],
    );
    expect(new Set(locators.map((locator) => locator.activeAsset))).toEqual(
      new Set(DIESEL_ENGINE_ASSETS),
    );
    expect(
      locators.every(
        (locator) =>
          [1, 2, 3, 4, 5].includes(locator.sourcePdfPage) &&
          locator.sourceRaster.width === 2320 &&
          locator.sourceRaster.height === 3408 &&
          locator.sourceRectPixels.x === 0 &&
          locator.sourceRectPixels.y === 0 &&
  ...
  test("does not promote US 7,479,949 preserved crops into active archival citations", () => {
    expect(FIGURE_OCCURRENCE_SOURCE_LOCATORS[MULTI_TOUCH_ID]).toBeUndefined();
    expect(ARCHIVAL_FIGURE_ACCEPTANCE_ATTESTATIONS[MULTI_TOUCH_ID]).toBeUndefined();
  });
