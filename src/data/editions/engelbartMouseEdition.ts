...
export const engelbartMouseArchivalEdition: CuratedSpecificationEdition = {
  ...
  blocks: [
    ...
    p([
      ...
      figure("FIG. 3", 3),
      text(" is a sectional plan view of the mechanism of FIG. 2; "),
      figure("FIG. 4", 4),
      text(
    ...
    p(
      "In order to indicate many positions, the disc 80 is, in practice divided into a large number of rings and sectors, so that a large number of positions can be indicated and small changes of position are registered. A similar scheme is used for the Y position. The advantage of the readout scheme of FIG. 6 is that a digital output is provided which completely defines the position of the indicator at every instant. A major disadvantage is that a large number of wires must be connected to the position indicator control so that a relatively thick cable trails behind it and limits the ease with which it can be moved.",
    ),
    p([
      figure("FIG. 6", 6),
      text(
        " illustrates still another position readout means, which possesses the advantage of digital output while requiring a minimum number of leads connecting the position indicator control to the computer. In the readout circuit of FIG. 5, a disc 100 is provided which has three rows of electrical contacts, designated 102, 104 and 106. The disc 100 has its axes fixed to the X wheel of the device shown in FIGS. 1, 2 and 3 in place of the potentiometer. The device of FIG. 6 operates by transmitting “up” pulses when the position indicator control moves to the right and “down” pulses when the position indicator control moves to the left. A counter circuit, which is fixed to the computer adds the up pulses and subtracts the down pulses to provide a continuous digital indication of the position of the position indicator control. A similar arrangement is used for the Y position.",
      ),
    ]),
    p(
      "In the circuit of FIG. 6 three sensors 108, 110 and 112 are located adjacent to the rows of contacts 102, 104 and 106, respectively. Whenever the disc 100 rotates and one of the contacts of one of the three rows comes under a sensor, a voltage is delivered over one of the lines 114, 116 or 118, leading to circuitry located at the computer. The contacts on the three rows 102, 104 and 106 are located so that only one contact is under a sensor at any given time.",
    ),
    p(
    ...
    p(
      "The circuit of FIG. 6 requires only four leads between the computer and the position indicator control. The four leads are the lines 114, 116, 118, and an additional line (not shown) for connection to a voltage source to provide pulses that flow through the sensors to the other three lines.",
    ),
    p([
      figure("FIG. 7", 7),
      text(
        " illustrates another position readout means of the incremental encoder type, similar to that of FIG. 6. The circuit of FIG. 7 utilizes a simpler encoding disc and one fewer lead, although it involves more complex electronics. In the circuit of FIG. 7, a disc 140 whose axis is connected to a position wheel such as the X position wheel is provided which has a track 142 having spaced conductive segments. A control contact 146 and stepping contact 148 are disposed over the track to make contact with the conductive segments thereof. The contacts are arranged for contacting the segments at angular positions of the disc which overlap. A lead (not shown) connected to the disc 140 conducts currents to the segments of the two tracks.",
      ),
    ]),
    p(
    ...
    p(
      "The particular mechanical construction shown in FIGS. 2 and 3 are especially well adapted for maintaining accuracy of output and ease of use. The use of only three points of contact, comprising the two wheels and the ball bearing support, help to assure that both wheels will constantly remain in firm contact with the surface on which the position indicator control rests. The location of the various buttons for indicating areas of the display to be operated on, or for other purposes, on the indicator which is moved by the hand allows a human operator to maintain control over both position of changes and the type of changes on the display with only one hand. The use of an indicator control which rests firmly on a surface enables the operator to accurately maintain position with a minimum of muscle effort, since the indicator control remains stationary unless some force is applied to it. The use of relatively large position wheels having appreciable, even if small, moments of inertia, reduces jittering of the indicator control and promotes smooth movement which is helpful in accurate positioning where the displayed characters are small or where accurate tracing of a pattern is required.",
    ),
    p(
