...
describe("US 3,260,375 Lemelson Adjustable Manipulator Archival Edition Contract", () => {
  ...
  test("uses real local source crops, term annotations, and an honest historical boundary", () => {
    for (const figure of [1, 2, 3]) {
      const crop = resolve(
        process.cwd(),
        `public/patents/figures/us-3260375-lemelson-adjustable-manipulator/fig-${figure}-source-crop-v1.png`,
      );
      expect(existsSync(crop)).toBe(true);
      const bytes = readFileSync(crop);
