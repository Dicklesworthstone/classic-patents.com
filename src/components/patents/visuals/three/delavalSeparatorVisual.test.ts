...
describe("US 247,804 Gustaf de Laval Centrifugal Cream Separator visual & fluid dynamics boundary", () => {
  ...
  test("offers exactly one live bowl-speed control alongside the raw-milk feed control", () => {
    ...
    expect(threeSource).toContain('paramKey="bowlRpm"');
    expect(threeSource).toContain('paramKey="rawMilkFlowLph"');
    expect(threeSource).toContain("<SensitivitySlider");
  });
  ...
  test("computes genuine centrifugal acceleration and fat yield percentage in SI units", () => {
    ...
    expect(result.creamDropWrap).toBeCloseTo(1.8, 3);
    expect(result.skimDropWrap).toBeCloseTo(2.0, 3);
  });
