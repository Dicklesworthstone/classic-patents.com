...
import { Camera, Layers, RotateCcw, Sparkles, Volume2, VolumeX, Zap } from "lucide-react";
...
const SCENARIOS: ScenarioPreset[] = [
  {
    id: "raw",
    name: "Raw Natural Gum Rubber",
    desc: "Unvulcanized cis-1,4-polyisoprene. Sticky in heat, brittle in winter, permanent plastic slippage.",
    sulfurPct: 0.5,
    tempC: 25,
    stretch: 1.8,
  },
  {
    id: "vulcanized",
    name: "Goodyear 1844 Vulcanization",
    desc: "Optimal sulfur cross-linking at 140°C. High entropic elasticity with 100% reversible snapback.",
    sulfurPct: 4.5,
    tempC: 140,
    stretch: 2.2,
  },
  {
    id: "ebonite",
    name: "Hard Ebonite / Vulcanite",
    desc: "Heavy sulfur loading (12%+). Highly rigid polymer network with ceramic-like hardness.",
    sulfurPct: 12.0,
    tempC: 160,
    stretch: 1.15,
  },
  {
    id: "cryo",
    name: "Cryogenic Glass Transition",
    desc: "Sub-zero temperatures freeze molecular rotation below Tg (-70°C), inducing brittle shattering.",
    sulfurPct: 4.5,
    tempC: -40,
    stretch: 1.05,
  },
        ...
        <div className="absolute top-4 left-4 z-10 flex flex-col gap-2 pointer-events-none max-w-[calc(100%-8rem)] sm:max-w-md">
          <div className="bg-white/90 dark:bg-ink-900/90 backdrop-blur-md px-3.5 py-2.5 rounded-xl border border-parchment-300 dark:border-ink-700 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 mt-1 text-xs font-sans">
              ...
              <div>
                <span className="text-ink-600 dark:text-ink-400">Applied Stretch ($\lambda$):</span>{" "}
                <span className="font-bold text-emerald-600 dark:text-emerald-400">
                  {appliedTensileStretch.toFixed(2)}× ({(appliedTensileStretch * 100).toFixed(0)}%)
                </span>
              </div>
              <div>
                <span className="text-ink-600 dark:text-ink-400">True Tensile Stress:</span>{" "}
                <span className="font-bold text-purple-600 dark:text-purple-400">
                  {trueStressMpa} MPa
                </span>
              </div>
              <div>
                <span className="text-ink-600 dark:text-ink-400">Entropic Force ($\Delta S$):</span>{" "}
          ...
          <div className="bg-white/90 dark:bg-ink-900/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-parchment-300 dark:border-ink-700 text-[11px] font-sans text-ink-700 dark:text-ink-300 flex items-center gap-2 max-w-full">
            ...
            <span className="truncate">
              Charles Goodyear (US 3,633) — Covalent Sulfur Vulcanization
            </span>
          </div>
        </div>

        {/* Top Right Tool Bar (Audio, Callouts & Reset) */}
        {/* Top Right Tool Bar (Audio, Callouts & Reset) */}
