...
export function stepDieselEngine(params: DieselEngineStepParams) {
  ...
  return {
    ...
    crankOmegaSlopeRadPerSPerRpm: Math.PI / 30,
    crankOmegaSlopeDegPerSPerRpm: 6.0,
    blastPressureMarginBar: pBlast - pCompBar,
    blastPressureMarginSlopeBarPerBar: 1.0,
    governorBallSpread: Number(Math.min(1.4, Math.max(0.4, (rpm / 150) * 0.85)).toFixed(3)),
    governorBallSpreadSlopePerRpm: 0.85 / 150,
