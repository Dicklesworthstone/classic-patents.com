...
describe("executable project coverage manifest", () => {
  test("contains one evidence row per catalogue record", () => {
    ...
    expect(new Set(manifest.map((row) => row.patentId)).size).toBe(87);
    expect(manifest.every((row) => row.source.pinnedFacsimile)).toBe(true);
    expect(manifest.filter((row) => row.source.reviewedLedger)).toHaveLength(84);
    expect(manifest.filter((row) => row.source.archivalEdition === "published")).toHaveLength(73);
  ...
  test("distinguishes patent-specific, interpretive, generic, and typed-host surfaces", () => {
    ...
    expect(manifest.filter((row) => row.runtime.wasmSurface === "none")).toHaveLength(44);
  ...
  test("all 3D studios now have an updater or a typed snapshot path", () => {
    ...
    expect(manifest.filter((row) => row.runtime.sharedBus === "snapshot")).toHaveLength(40);
