...
- [x] P5.445 Hewitt 2D condensate drops drain `plasmaFlickerOmegaRadPerS`; starting-kick jolt drains `strikeJoltOmegaRadPerS` (4/3 of flicker). Kilby 2D bond-pulse travel drains `bondPulseAdvance` (`ω_switch × 0.25`). Haber 2D catalyst rise / condenser drip drain `catalystParticleAdvance` / `condenserDripAdvance` from compressor ω. Tesla teleautomaton RF rings drain `cohererDisplayOmegaRadPerS × 4/3` (2.0 when locked, 0 when detuned). Leftover `time * 30` / `40` / `1.5` / `40` / `60` / `timeSec * 2.0` are gone. P6.5 still does not copy the 5.1 MB `fs-wasm` artifact.
- [x] P5.446 Bardeen hole-drift pads drain named `BARDEEN_HOLE_WRAP_PAD` / `BARDEEN_HOLE_RESET_PAD` instead of leftover `stepBardeenTransistor()`. E-Ink 2D thermal jitter drains Stokes-Einstein `brownianJitterOmegaYRadPerS` / `X` (2.0 cP → leftover 2.3 / 1.7; thicker fluid slows Brownian motion). Cort puddle flicker / spark hash drain mill ω (`rabble × 10/π` → leftover 5 at 15 rpm; `roll × 10/π` → leftover 10 at 30 rpm). Linde mist golden-angle spawn scales with grant `p` (`timeSec × 10 × pressureNorm`). Hall CO₂-bubble lift / sway / anode pulse drain current density (300 kA → leftover 0.4 / 3 / 1.5; I = 0 freezes the bath). Crate-tape `timeSec × 8 % 16` and Tesla hull heave stay presentation. P6.5 still does not copy the 5.1 MB `fs-wasm` artifact.
- [x] P5.447 Fessenden 2D leftover `t × 1.8` / `8` / `50` / `20` / `40` drain kernel `waveRingDisplayRate` / `audioEnvelopeOmegaRadPerS` / `rfTraceDisplayOmegaRadPerS` / `barretterGlowOmegaRadPerS` / `telephoneRingDisplayOmegaRadPerS` (75 kHz / 1 kHz audio → leftover 1.5 / 6 / 50 / 20 / 40). PageRank surfers drain `omegaRadPerSec` (`d/0.85 × 0.8`; d = 0 freezes). Roomba side-brush ω drains wheel speed (0.3 m/s → leftover 18). Carrier mist spawn hash drains CFM instead of leftover `dt × 100`. E-Ink 3D capsule jitter drains the Stokes-Einstein mean ω (2.0 cP → leftover 2). Tesla hull heave and crate-tape sampling stay presentation. P6.5 still does not copy the 5.1 MB `fs-wasm` artifact.
...
- [x] P7.11 Host-pumped `TickScheduler` / `createStudioClock` on the catalog 3D
